  ...
  try {
    ...
    try {
      ...
      if (roleResult.rowCount === 0) {
        ...
        return res
          .status(500)
          .json({ message: "Doctor role not found in system" });
      }
      await client.query(
...
router.patch("/:id", authMiddleware, requireRole("admin"), async (req, res) => {
  ...
  if (!name && !email && !specialization) {
    return res
      .status(400)
      .json({ message: "name, email or specialization is required" });
  }
  ...
  try {
    ...
    if (specialization) {
      await pool.query(`UPDATE doctors SET specialization = $1 WHERE id = $2`, [
        specialization,
        doctorId,
      ]);
    }
    ...
    try {
      ...
      try {
        ...
        if (doctorCheck.rows[0].active === false) {
          ...
          return res
            .status(409)
            .json({ message: "Doctor is already suspended" });
        }

        const doc = doctorCheck.rows[0];

        const removeWards = await client.query(
          `DELETE FROM doctor_wards WHERE doctor_id = $1`,
          [doctorId],
        );
    ...
    try {
      ...
      try {
        ...
        await notifyUsersBestEffort(
          [...adminUserIds, req.user.id, doc.user_id],
          {
...
router.post(
  ...
  async (req, res) => {
    ...
    if (Number.isNaN(doctorId) || Number.isNaN(wardId)) {
      return res
        .status(400)
        .json({ message: "doctor id and ward_id are required" });
    }
    ...
    try {
      ...
      if (doctorCheck.rows[0].active === false) {
        return res
          .status(409)
          .json({ message: "Cannot assign a suspended doctor to a ward" });
      }
      ...
      if (insertResult.rowCount === 0) {
        return res
          .status(409)
          .json({ message: "Doctor is already assigned to this ward" });
      }

      try {
        const adminUserIds = await listUserIdsForRole("admin");
        await notifyUsersBestEffort(
          [...adminUserIds, req.user.id, doctorCheck.rows[0].user_id],
          {
            ...
            metadata: {
              doctor_id: doctorId,
              doctor_user_id: doctorCheck.rows[0].user_id,
              ward_id: wardId,
