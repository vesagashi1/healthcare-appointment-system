  ...
  fileFilter: (req, file, cb) => {
    ...
    if (allowedExtensions.includes(fileExtension)) {
      ...
      if (allowedTypes.includes(file.mimetype)) {
        ...
        cb(
          new Error(
            `Invalid file type. Only CSV, Excel, and JSON are allowed. Got: ${file.mimetype}`,
          ),
        );
      }
    }
    ...
    return data.map((row) => {
      if (row.date_of_birth && typeof row.date_of_birth === "number") {
        ...
        const jsDate = new Date(
          excelEpoch.getTime() + row.date_of_birth * 86400000,
        );
    ...
    try {
      if (!req.file) {
        return res
          ...
          .json({
            message:
              "No file uploaded. Make sure to select 'File' type in Postman form-data.",
          });
      }
      ...
      console.log(
        `Success count: ${results.success.length}, Error count: ${results.errors.length}`,
      );
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (!row.patient_id || !row.doctor_id || !row.appointment_date) {
          results.errors.push({
            ...
            data: row,
            errors: [
              "Missing required fields: patient_id, doctor_id, appointment_date",
            ],
          });
          continue;
        }
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (!row.patient_id || !row.record_type || !row.content) {
          results.errors.push({
            ...
            data: row,
            errors: [
              "Missing required fields: patient_id, record_type, content",
            ],
          });
          continue;
        }
