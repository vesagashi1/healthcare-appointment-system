...
import {
  HeartPulse, X, Building2, Users, Plus, Pencil, Ban, RotateCcw, Trash2,
} from 'lucide-react';
...
interface NurseWard { id: number; name: string; }
interface NursePatient { id: number; user_id: number; name: string; email: string; ward_id: number | null; ward_name: string | null; }
...
const NursesPage = () => {
  ...
  const openDetail = async (nurseUserId: number) => {
    ...
    } catch (err) { console.error('Error fetching nurse details:', err); }
    finally { setLoadingDetail(false); }
  };

  const closeDetail = () => { setShowDetail(false); setSelectedNurse(null); };
  ...
  const openEdit = (nurse: NurseListItem) => {
    ...
    setFormName(nurse.name); setFormEmail(nurse.email); setFormPassword(''); setFormError('');
    setShowForm(true);
  };

  const closeForm = () => { setShowForm(false); setEditingNurse(null); };
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingNurse) {
        await api.patch(`/nurses/${editingNurse.user_id}`, { name: formName, email: formEmail });
      } else {
        if (!formPassword) { setFormError('Password is required for new nurse'); setFormSubmitting(false); return; }
        await api.post('/nurses', { name: formName, email: formEmail, password: formPassword });
      }
      closeForm();
      await fetchNurses();
      // Refresh detail if open
      if (showDetail && selectedNurse && editingNurse?.user_id === selectedNurse.id) {
  ...
  const handleSuspend = async (nurseId: number) => {
    if (!confirm('Suspend this nurse? Their ward and patient assignments will be removed.')) return;
    try {
      await api.delete(`/nurses/${nurseId}`);
      await fetchNurses();
      if (showDetail && selectedNurse?.id === nurseId) openDetail(nurseId);
    } catch (err: any) { alert(err?.response?.data?.message || 'Failed to suspend nurse'); }
  };
  ...
  const handleRestore = async (nurseId: number) => {
    try {
      ...
      await fetchNurses();
      if (showDetail && selectedNurse?.id === nurseId) openDetail(nurseId);
    } catch (err: any) { alert(err?.response?.data?.message || 'Failed to restore nurse'); }
  };
  ...
  const handleAssignWard = async () => {
    ...
    setAssignError('');
    try {
      await api.post(`/nurses/${assignNurseId}/wards`, { ward_id: selectedWardId });
      setShowAssignWard(false);
      await fetchNurses();
      if (showDetail && selectedNurse?.id === assignNurseId) openDetail(assignNurseId);
    ...
    <div>
      ...
      {nurses.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {nurses.map((nurse) => (
            <div key={nurse.user_id} className={`card hover:shadow-lg transition-shadow ${nurse.active === false ? 'opacity-60' : ''}`}>
              <button type="button" onClick={() => openDetail(nurse.user_id)} className="text-left w-full">
                <div className="flex items-start justify-between mb-4">
                  <div className="min-w-0">
                    <h3 className="text-xl font-semibold text-slate-100 truncate">{nurse.name}</h3>
                    <p className="text-sm text-slate-300 truncate">{nurse.email}</p>
                  </div>
                  <div className={`p-2 rounded-full border ${nurse.active === false ? 'bg-red-500/15 border-red-500/25' : 'bg-teal-500/15 border-teal-500/25'}`}>
                    <HeartPulse className={`h-6 w-6 ${nurse.active === false ? 'text-red-300' : 'text-teal-300'}`} />
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  <span className="inline-flex items-center gap-1 px-3 py-1 bg-teal-500/15 text-teal-200 border border-teal-500/30 rounded-full text-sm font-medium">
                    ...
                    {Number(nurse.ward_count)} ward{Number(nurse.ward_count) !== 1 ? 's' : ''}
                  </span>
                  {nurse.active === false && (
              ...
              {isAdmin && (
                <div className="flex gap-2 mt-4 pt-3 border-t border-slate-800">
                  <button onClick={() => openEdit(nurse)} className="flex items-center gap-1 text-sm text-slate-300 hover:text-blue-300 transition-colors" title="Edit">
                    <Pencil className="h-3.5 w-3.5" /> Edit
                  </button>
                  {nurse.active !== false && (
                    <button onClick={() => openWardAssign(nurse.user_id)} className="flex items-center gap-1 text-sm text-slate-300 hover:text-teal-300 transition-colors" title="Assign Ward">
                      <Building2 className="h-3.5 w-3.5" /> Assign Ward
                    </button>
                  )}
                  {nurse.active === false ? (
                    <button onClick={() => handleRestore(nurse.user_id)} className="flex items-center gap-1 text-sm text-slate-300 hover:text-green-300 transition-colors ml-auto" title="Restore">
                      <RotateCcw className="h-3.5 w-3.5" /> Restore
                    </button>
                  ) : (
                    <button onClick={() => handleSuspend(nurse.user_id)} className="flex items-center gap-1 text-sm text-slate-300 hover:text-red-300 transition-colors ml-auto" title="Suspend">
          ...
          {loadingDetail ? (
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="text-lg font-semibold text-slate-100">{selectedNurse.name}</h3>
                  <p className="text-sm text-slate-400">{selectedNurse.email}</p>
                  <p className="text-xs text-slate-500 mt-1">Joined {new Date(selectedNurse.created_at).toLocaleDateString()}</p>
                </div>
              ...
              <div className="flex gap-3">
                <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-teal-500/15 text-teal-200 border border-teal-500/30 rounded-full text-sm font-medium">
                  ...
                  {selectedNurse.stats.ward_count} ward{selectedNurse.stats.ward_count !== 1 ? 's' : ''}
                </span>
                <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-500/15 text-blue-200 border border-blue-500/30 rounded-full text-sm font-medium">
                  ...
                  {selectedNurse.stats.patient_count} patient{selectedNurse.stats.patient_count !== 1 ? 's' : ''}
                </span>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  {isAdmin && selectedNurse.active !== false && (
                    <button onClick={() => openWardAssign(selectedNurse.id)} className="text-xs text-teal-400 hover:text-teal-300">+ Assign Ward</button>
                  )}
                </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedNurse.wards.map((ward) => (
                      <span key={ward.id} className="inline-flex items-center gap-1 px-3 py-1 bg-slate-800 border border-slate-700 rounded text-sm text-slate-200">
                        ...
                        {isAdmin && (
                          <button
                            onClick={() => handleUnassignWard(selectedNurse.id, ward.id)}
              ...
              {/* Assigned patients */}
              <div>
                <h4 className="text-sm font-semibold text-slate-300 uppercase tracking-wider mb-2">Assigned Patients</h4>
            ...
            <p className="text-slate-400 text-center py-8">Failed to load nurse details.</p>
          )}
        </Modal>
      )}
      ...
      {/* ── Create / Edit modal ── */}
      {showForm && (
        <Modal title={editingNurse ? 'Edit Nurse' : 'Add Nurse'} onClose={closeForm}>
          <form onSubmit={handleFormSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input type="email" className="input-field w-full" required value={formEmail} onChange={(e) => setFormEmail(e.target.value)} placeholder="nurse@example.com" />
            </div>
            {!editingNurse && (
              <div>
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button type="submit" disabled={formSubmitting} className="btn-primary">
                {formSubmitting ? 'Saving…' : editingNurse ? 'Save Changes' : 'Create Nurse'}
              </button>
            </div>
          </form>
        </Modal>
      ...
      {/* ── Assign Ward modal ── */}
      {showAssignWard && (
        <Modal title="Assign Nurse to Ward" onClose={() => setShowAssignWard(false)}>
