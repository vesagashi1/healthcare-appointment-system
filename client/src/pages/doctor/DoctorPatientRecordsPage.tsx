...
import { createPatientRecord, getPatientRecords, type PatientRecord } from '../../shared/api/patientApi';
...
export function DoctorPatientRecordsPage() {
  ...
  const create = async (e: FormEvent) => {
    ...
    setError(null);
    try {
      await createPatientRecord(token, patientId, { record_type: recordType, content });
      setContent('');
      await load();
    } catch (err) {
      if (err instanceof HttpError) setError(err.message);
  ...
  return (
    <div style={{ maxWidth: 900, margin: '32px auto', padding: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'end', flexWrap: 'wrap' }}>
        <label>
          Patient ID
          <input value={patientIdText} onChange={(e) => setPatientIdText(e.target.value)} placeholder="e.g. 12" />
        </label>
          ...
          {records.map((r) => (
            <div key={r.id} style={{ border: '1px solid #ddd', borderRadius: 8, padding: 12 }}>
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                ...
                <span>created_at: {new Date(r.created_at).toLocaleString()}</span>
              </div>
              <pre style={{ whiteSpace: 'pre-wrap', marginTop: 8 }}>{r.content}</pre>
            </div>
          ))}
        </div>
      )}
